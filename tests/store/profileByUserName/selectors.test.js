...
import { getProfileByUserName, getProfileByUserNameState } from '../../../src/store/profileByUserName/selectors';
...
describe('Test getProfileByUserName selector', () => {
    it('Should return result of type object', () => {
        ...
        const selected = getProfileByUserName.resultFunc(mockProfile);
        expect(type(selected[0])).toEqual("Object");
        expect(selected[0].FullName).toEqual("Santa Claus")
    });

    it("Should return getProfileByUserName state", () => {
        const state = getProfileByUserNameState();
